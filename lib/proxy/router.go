...
type RouterConfig struct {
	...
	// Tunnel allows looking up sites for connecting to other clusters
...
func (c *RouterConfig) CheckAndSetDefaults() error {
	...
	// it is possible for the reverse tunnel server to be disabled in tests
	// log a warning
	if c.Tunnel == nil {
		c.Log.Warn("no reverse tunnel provided, is the reverse tunnel server enabled?")
	}
...
func NewRouter(cfg RouterConfig) (*Router, error) {
	...
	// it is possible for the reverse tunnel server to be disabled in tests
	var localSite reversetunnel.RemoteSite
	if cfg.Tunnel != nil {
		site, err := cfg.Tunnel.GetSite(cfg.ClusterName)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		localSite = site
